...
#metodos de creacion de FICHEROS DE OUTPUT
...
#METODO GENERAL QUE EJECUTA EL MAIN HABRIA QUE PASARLO A UNA CLASE PARA QUE TENGA LA MISMA ARQUITECTURA DEL CODIGO.
...
class CalculateScoresClass():
    ...
    def fntProcessSubmissions(self, submissionsPerLanguagePerYear, overwrite_all=False, bAverageFiles=False,
                                  fpl=None):
            cross = False
            if fpl is not None:
                global filesPerLanguage
            ...
            for (lang_pair, years) in submissionsPerLanguagePerYear.items():
                ...
                for (year, info) in submissionsPerLanguagePerYear[lang_pair].items():
                    ...
                    # Calculate all the PPL for a given target domain and year
                    # ToDo: Check that output files do not exist to avoid entering here
                    calculateLMMetrics(tgt, lang_pair, year,
                                       submissionsPerLanguagePerYear[lang_pair][year]['submissions'])
                    ...
                    for filename in info['submissions']:
                        ...
                        if cross is True:
                            ...
                            am_cross_scores_sent = {}
                            print(
                                'AM Cross scores for year:' + year + ' lang_pair:' + lang_pair + ' file:' + filenamebase)
                            print('Calculating am cross scores')
                            tmpParams = []
                            for train_size in aSizesTrain:
                                nameModel = cross_model_path + '.' + str(train_size) + '.am_cross_model' + exTAvg
                                if overwrite_all is True or not os.path.exists(nameModel):
                                    tmpParams.append(
                                        (train_size, src, tgt, year, filename, source_file, cross_model_path,
                                         bAverageFiles))
                            if len(tmpParams) > 0:
                                pool = multiprocessing.Pool(processes=NUM_MAX_CORES)
                                pool.map(am_cross_scores_wrapper, tmpParams)
                        ...
                        for i in range(len(fm_scores_sent[MIN_NGRAM_ORDER])):
                            ...
                            if bAverageFiles is False:
                                ...
                                for train_size in aSizesTrain:
                                    for key in sorted(am_mono_scores_sent[train_size]):
                                        sent += str(num_feature) + ':' + str(
                                            am_mono_scores_sent[train_size][key][i]) + " "
                                        num_feature += 1
                                    if cross is True:
                                        for key in sorted(am_cross_scores_sent[train_size]):
                                            sent += str(num_feature) + ':' + str(
                                                am_cross_scores_sent[train_size][key][i]) + " "
                                            num_feature += 1
    ...
    def am_mono_scores_wrapper(self,args):
        return calculateMonolingualScores(*args)

    def am_cross_scores_wrapper(self,args):
        return calculateCrosslingualScores(*args)

    def lm_wrapper(self,args):
        return calculateLMMetricsPerTaskParallel(*args)

    def am_scores_wrapper(self,args):
        return calculateMonolingualScores(*args)

    # metodos de creacion de FICHEROS DE OUTPUT

    def createAndcheckOutputDirs(self):
        print("***** Checking input and creating output directories *****")
        if not os.path.exists(dir_svd_mono):
            print("ERROR: the svd dir for the monolingual matrices does not exist")
            exit(-1)
    ...
    def calculateLMMetricsPerTaskParallel(self,year, tgt, filename, ngram_order):
        print("Calculating LM Metrics for year: " + year + " lang:" + tgt + "filename: " + filename + " and ngram:" +
              str(ngram_order) + " ******")
        t1 = datetime.datetime.now()
        lm_scores_systems = []
    ...
    def calculateMonolingualScores(self,size, src, tgt, year, filename, reference_file, mono_model_path,
                                   bAverageFiles=False):
        print('Calculating mono AM scores for: size:' + str(size) + ' year:' + str(year) +
              ' lang_pair:(' + src + '-' + tgt + ') file:' + filename)
        t1 = datetime.datetime.now()
        ...
        assert len(test_sentences) == len(
            ref_sentences), "******* ERROR: sentence lengths are not the same for files %s " \
                            "(%d) and %s (%d)" % (root_dir + '/' + filename + '.lower',
                                                  len(test_sentences),
                                                  root_dir + reference_file,
                                                  len(ref_sentences))
        ...
        for (id_file, svd_file) in enumerate(filesPerLanguage[src + '-' + tgt]):
            for nF in range(NFOLDS):
                ...
                if os.path.isfile(svd_full_matrix + '.h5') and os.path.isfile(svd_full_matrix + '.dic'):
                    ...
                    print(
                        '******* ERROR: files: ' + svd_full_matrix + '.h5 or ' + svd_full_matrix + '.dic do not exists.')
                    return
                    # exit(-1)
    ...
    def calculateCrosslingualScores(self,size, src, tgt, year, filename, source_file, cross_model_path, bAverageFiles=False):
        print('Calculating cross AM scores for: size:' + str(size) + ' year:' + str(year) +
              ' lang_pair:(' + src + '-' + tgt + ') file:' + filename)
        t1 = datetime.datetime.now()
    ...
    def calculateLMMetrics(self,tgt, lang_pair, year, submissions):
