...
class TrainingClass():
    def __init__(self, train_data_dir, filesPerLanguageForLM, filesPerLanguage, dictSizesTrain, cfg):
        ...
        self.dir_lm_out = cfg['trainModule']['dir_lm_out']
        self.dir_svd_mono = cfg['trainModule']['dir_svd_mono']
        self.type_vectorizer = cfg['trainModule']['type_vectorizer']
        self.cfg = cfg
    ...
    def createOutputDirs(self):
        ...
        if not os.path.exists(self.dir_lm_out):
            print("...creating " + self.dir_lm_out)
            os.makedirs(self.dir_lm_out)

        if not os.path.exists(self.dir_svd_mono):
            print("...creating " + self.dir_svd_mono)
            os.makedirs(self.dir_svd_mono)
    ...
    def createLM(self, train_data_dir, lang, filename, bRetrain=False):
        ...
        if not os.path.exists(self.dir_lm_out + path_file + '/' + lang):
            ...
            os.makedirs(self.dir_lm_out + path_file + '/' + lang)

        for ngram_order in range(MIN_NGRAM_ORDER, MAX_NGRAM_ORDER + 1, 1):
            lm_file_out = self.dir_lm_out + path_file + '/' + lang + '/' + os.path.basename(
    ...
    def createMonolingualSVDFromTrainingFiles(self, size_file, size_svd, nF, retrain_svd):
        ...
        for (lang_pair, files) in self.filesPerLanguage.items():
            ...
            for file in files:
                ...
                if not os.path.exists(self.dir_svd_mono + path_file):
                    print("...creating " + self.dir_svd_mono + path_file)
                    os.makedirs(self.dir_svd_mono + path_file)
    ...
    def createMonolingualSVDforTaskParallel(self, file, size_file, size_svd, nF, lang, retrain_svd):
        ...
        svd_output_matrix = self.dir_svd_mono + '/' + file + '.' + lang + '.' + str(size_file) + '.' + str(
            size_svd) + '.' + str(nF)
        ...
        if os.path.isfile(train_file_path):
            if (not os.path.isfile(svd_output_matrix + '.h5') or not os.path.isfile(svd_output_matrix + '.dic')) \
                ...
                vs.trainVectorSpace((size_svd), lang, MIN_COUNTS, train_sentences,
                                    transforms=self.type_vectorizer)  # Names of the output files
    ...
    ####### METHODS ONLY USED BY THE MAIN OF THIS SCRIPT######
    def createTrainingFiles(self, overwrite_all=False):
        ...
        for (lang_pair, files) in self.filesPerLanguage.items():
            ...
            tgt = aPrefix[1]
            for file in files:
                tmpTask.append((self.train_data_dir + file, src, tgt, overwrite_all))
...
def main():
    ...
    train_data_dir = cfg['trainModule']['train_data_dir']
